...
command = "(time idr --samples "+\
          ...
          "  --rank q.value"+\
          " --output-file "+snakemake.params.all_bed+\
          " --plot "+\
...
command = "(time awk '$5 >= "+str(-125*math.log2(snakemake.params.cutof))+"' OFS='\t' FS='\t' "+snakemake.params.all_bed+" > "+snakemake.output.bed+") 2>> "+snakemake.log.run
f = open(snakemake.log.run, 'at')
f.write("## COMMAND: "+command+"\n")
f.close()
shell(command)
