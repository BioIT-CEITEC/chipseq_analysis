...
if snakemake.wildcards.tool == "MACS":
  # input is ranked by q.value (i.e. from MACS)
  ...
  # input is ranked by score (e.g. from SEACR)
  command = "$(which time) idr --samples "+\
            " <(sort -k5,5nr -T "+snakemake.params.tmpd+" "+snakemake.input.peaks[0]+")"+\
            " <(sort -k5,5nr -T "+snakemake.params.tmpd+" "+snakemake.input.peaks[1]+")"+\
            " --input-file-type bed"+\
            " --rank score"+\
            " --output-file "+snakemake.output.all_bed+\
            " --plot "+\
