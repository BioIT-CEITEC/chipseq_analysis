...
if os.path.isfile(snakemake.input.bed) and os.path.getsize(snakemake.input.bed) == 0:
    ...
    pdfkit.from_string('Input file '+snakemake.input.bed+' is missing or empty!', snakemake.output.profile)

    command = "touch "+snakemake.output.profile_data+" >> "+snakemake.log.run+" 2>&1"
    f = open(snakemake.log.run, 'at')
