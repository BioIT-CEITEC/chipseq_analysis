...
for(bed in beds) {
  ...
  comp = ifelse(grepl('ChIP-R', source), 'all_reps', 'rep1_VS_rep2')
  if(grepl('_VS_', name)) {
    comp = sub('.+\\.(rep[0-9]+_VS_rep[0-9]+)\\..+','\\1',name, perl=T)
  }
  new_tab = fread(bed, sep='\t',header = F)
  tab = rbind(tab, data.table(condition=cond, comparison=comp, source=source, reps_type=rtype, N=new_tab[,.N]))
  cat("## new bed count: ",new_tab[,.N],"\n")
}
print(tab)
